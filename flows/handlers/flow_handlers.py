    ...
    try:
        ...
        if not health_centers:
            ...
            if address_retry_count == 0:
                # First failure: ask patient to re-state address
                flow_manager.state["address_retry_count"] = 1
                logger.warning(f"⚠️ Address '{address}' not recognized, asking patient to retry (attempt {address_retry_count + 1})")
                ...
                # Second failure: offer transfer to operator
